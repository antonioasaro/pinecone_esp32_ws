...
#include "pid_ctrl.h"
#ifdef ANTONIO
// extern void publish_rosout(char *msg_name, int32_t msg_data);
#endif
...
int32_t BDC_PID_EXPECT_SPEED = 0;       // expected motor speed, in the pulses counted by the rotary encoder
int32_t RIGHT_BDC_PID_EXPECT_SPEED = 0; // expected motor speed, in the pulses counted by the rotary encoder
#else
...
static int32_t right_motor_encoder_count = 0;
#endif
...
#ifdef ANTONIO
    motor_encoder_count = cur_pulse_count;
...
{
    ESP_LOGI(TAG, "Requesting left wheel encoder status: %d", (int)motor_encoder_count);
...
void app_main(void)
{
...
#ifdef ANTONIO
    ESP_LOGI(TAG, "Create 2nd DC motor");
    bdc_motor_config_t right_motor_config = {
        .pwm_freq_hz = BDC_MCPWM_FREQ_HZ,
        .pwma_gpio_num = RIGHT_BDC_MCPWM_GPIO_A,
        .pwmb_gpio_num = RIGHT_BDC_MCPWM_GPIO_B,
    };
    bdc_motor_mcpwm_config_t right_mcpwm_config = {
        .group_id = 1,
        .resolution_hz = BDC_MCPWM_TIMER_RESOLUTION_HZ,
    };
    bdc_motor_handle_t right_motor = NULL;
    ESP_ERROR_CHECK(bdc_motor_new_mcpwm_device(&right_motor_config, &right_mcpwm_config, &right_motor));
...
#ifdef ANTONIO
    ESP_LOGI(TAG, "Init 2nd pcnt driver to decode rotary signal");
    ...
    pcnt_chan_config_t right_chan_a_config = {
        .edge_gpio_num = RIGHT_BDC_ENCODER_GPIO_A,
        .level_gpio_num = RIGHT_BDC_ENCODER_GPIO_B,
    };
    ...
    pcnt_chan_config_t right_chan_b_config = {
        .edge_gpio_num = RIGHT_BDC_ENCODER_GPIO_B,
        .level_gpio_num = RIGHT_BDC_ENCODER_GPIO_A,
    };
...
#ifdef ANTONIO
    ESP_LOGI(TAG, "Create 2nd PID control block");
    ...
    right_motor_ctrl_ctx.pid_ctrl = right_pid_ctrl;

    ESP_LOGI(TAG, "Create a 2nd timer to do PID calculation periodically");
    ...
    ESP_LOGI(TAG, "Enable 2nd motor");
    ESP_ERROR_CHECK(bdc_motor_enable(right_motor));
    ESP_LOGI(TAG, "Forward 2nd motor");
    ESP_ERROR_CHECK(bdc_motor_forward(right_motor));

    ESP_LOGI(TAG, "Start 2nd motor speed loop");
